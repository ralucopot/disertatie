...
class Whale:
    ...
    def initRandomEnergy(self, num_households, charge_max, discharge_max):
        ...
        for i in range(num_households):
            self.transf_energy[i] = ((charge_max - discharge_max) * rnd_instance.random() + discharge_max)
...
class WOA:
    ...
    def computeBest(self, population_size: int, num_households:int, charge_max:float, discharge_max:float, max_iter:int):
        ...
        for i in range(population_size):
            population[i] = Whale(num_households, charge_max, discharge_max)
        ...
        while curr_iter < max_iter:
            ...
            for i in range(population_size):
                ...
                A = 2 * a - r_1 * a 
                C = 2 * r_2
                new_whale:Whale = Whale(num_households, charge_max, discharge_max)
                random_whale:Whale = Whale(num_households, charge_max, discharge_max)
